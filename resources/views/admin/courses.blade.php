                    ...
                    @foreach ($courses as $course)
                        <div class="col-12 mb-4">
                            <div class="card h-100 shadow-sm">
                                <div class="row g-0">
                                    <div class="col-md-4">
                                        <img src="{{ asset('img/' . $course->image) }}" class="img-fluid" alt="Imagen"
                                            style="height: 250px; width: 100%; object-fit: cover; min-height: 250px;">
                                    </div>
                                    <div class="col-md-8">
                                        <div class="card-body h-100 d-flex flex-column">
                                            <h5 class="card-title">{{ $course->name }}</h5>
                                            <p class="card-text">{{ $course->description }}</p>
                                            <p class="card-text"><small class="text-muted">Estatus: {{ $course->status }}</small></p>
                                            <p class="card-text"><small class="text-muted">Vistas:</small></p>
                                            <div class="mt-auto text-end">
                                                <button class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#updateCourse{{ $course->id }}">
                        ...
                        @include('admin/modals/courses/updateCourse')
                    @endforeach
                </div>
            </div>
        </div>
